@php
    ...
    $trustTitle = ($product->meta['trust_badges_title'] ?? null) ?: ($copy['trust_badges_title'] ?? 'Secure Checkout');
    $trustText = ($product->meta['trust_badges_text'] ?? null) ?: ($copy['trust_badges_text'] ?? '100% safe & protected payments');
    $deliveryPromise = ($product->meta['delivery_promise'] ?? null) ?: ($copy['delivery_promise'] ?? 'Estimated: 2-5 Business Days');
@endphp
