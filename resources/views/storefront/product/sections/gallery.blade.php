<div class="sf-pdp-gallery">
    @if ($product->primaryImage())
        <img src="{{ asset('storage/'.$product->primaryImage()->path) }}" class="main-img" alt="{{ $product->primaryImage()->alt_text ?? $product->name }}" loading="eager" fetchpriority="high" width="600" height="600">
    @else
    ...
    @if ($product->images->count() > 1)
        <div class="sf-pdp-thumbs">
            @foreach ($product->images as $i => $image)
                <img src="{{ asset('storage/'.$image->path) }}" alt="{{ $product->name }} - Image {{ $i + 1 }}" loading="lazy"
                     class="{{ $i === 0 ? 'active' : '' }}"
