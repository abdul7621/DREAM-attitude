...
@section('content')
<h1 class="h4 mb-3">{{ isset($shippingRule) ? 'Edit Shipping Rule' : 'Create Shipping Rule' }}</h1>
<div class="card shadow-sm p-4">
<form action="{{ isset($shippingRule) ? route('admin.shipping-rules.update', $shippingRule) : route('admin.shipping-rules.store') }}" method="post">
@csrf 
@if(isset($shippingRule)) @method('PUT') @endif

<div class="row mb-4">
    <div class="col-md-6 mb-3">
        <label class="form-label">Rule Name *</label>
        <input type="text" name="name" class="form-control" value="{{ old('name', $shippingRule->name ?? '') }}" required placeholder="e.g. Free COD for Gujarat">
    </div>
    <div class="col-md-3 mb-3">
        <label class="form-label">Priority <small class="text-muted">(Higher executes first)</small></label>
        <input type="number" name="priority" class="form-control" value="{{ old('priority', $shippingRule->priority ?? 0) }}" min="0">
    </div>
    <div class="col-md-3 mb-3 d-flex align-items-end">
        <div class="form-check mb-2">
            <input class="form-check-input" type="checkbox" name="is_active" value="1" id="isActive" {{ old('is_active', $shippingRule->is_active ?? true) ? 'checked' : '' }}>
...
<div id="conditions-container">
    @php
        $conditions = old('conditions', isset($shippingRule) ? $shippingRule->conditions->toArray() : []);
    @endphp
...
<div class="row g-3">
    <div class="col-md-6">
        @php
            $actionType = old('action_type', isset($shippingRule) ? $shippingRule->action?->type : 'flat');
            $actionValue = old('action_value', isset($shippingRule) ? $shippingRule->action?->value : '0.00');
        @endphp
...
<div class="mt-5">
    <button type="submit" class="btn btn-primary">{{ isset($shippingRule) ? 'Update Rule' : 'Create Rule' }}</button>
